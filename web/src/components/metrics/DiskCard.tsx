...
export function DiskCard({ stats, loading }: DiskCardProps) {
  if (loading && !stats) {
    return (
  ...
  const percent = stats?.disk.percent ?? 0;
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1">
          <div className="text-2xl font-bold metric-value mb-1" style={{ color }}>
            {formatBytes(stats?.disk.used ?? 0)}
          </div>
          <p className="text-xs mb-2" style={{ color: 'var(--color-text-muted)' }}>
            of {formatBytes(stats?.disk.total ?? 0)} on {stats?.disk.mount ?? '/'}
          </p>
          <div className="text-xs" style={{ color: 'var(--color-text-secondary)' }}>
            <span style={{ color: 'var(--color-text-muted)' }}>FS </span>
            <span className="metric-value">{stats?.disk.fs ?? '—'}</span>
          </div>
        </div>
      </div>
    </div>
